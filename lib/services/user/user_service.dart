...
class UserService {
  static void dashboardManagement(StackRouter router) async {
    ...
    if (roles.contains("ADMIN".toUpperCase())) {
      router.pushAndPopUntil(const AdminHomepageRoute(),
          predicate: (route) => true);
    } else if (roles.contains("STAFF".toUpperCase())) {
      router.pushAndPopUntil(const StaffHomepageRoute(),
          predicate: (route) => true);
    } else if (roles.contains("USER".toUpperCase())) {
      router.replaceAll([const UserLayoutRoute()]);
    } else {
      router.pushAndPopUntil(const LoginScreenRoute(),
          predicate: (route) => true);
    }
  }
}
