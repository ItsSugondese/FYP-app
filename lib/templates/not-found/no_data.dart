import 'dart:convert';

import 'package:flutter/material.dart';
import 'package:fyp/constants/designing/dimension.dart';

class NoData {
  static Widget getNoDataImage(
      BuildContext context, String? val, double? containerHeight) {
    return SizedBox(
      height: containerHeight != null
          ? Dimension.getScreenHeight(context) * containerHeight
          : null,
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          Image.memory(
            // Decode the Base64 string into bytes
            base64Decode(
                ""),
          ),
          Text(val == null ? "No Data" : val)
        ],
      ),
    );
  }
}
